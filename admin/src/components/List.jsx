...
const List = () => {
  ...
  const renderStars = (rating) =>
    [...Array(5)].map((_, i) => (
      <FiStar
        key={i}
        className={`text-xl drop-shadow-md ${i < rating ? 'text-amber-400 fill-current' : 'text-amber-100/30'
          }`}
      />
    ));
            ...
            <table className={styles.table}>
              <thead className={styles.thead}>
                <tr>
                  ...
                  <th className={styles.th}>Price (Tk)</th>
              ...
              <tbody >
                {items.map(item => (
                  <tr key={item._id} className={styles.tr}>
                    ...
                    <td className={styles.priceCell}>Tk{item.price}</td>
